...
/-- Lengths of walks satisfying a predicate `P`, bounded by the number of edges.
This is the common finite set behind the maximal-trail constructions below. -/
private noncomputable def trailLengthSetFor
    (P : ∀ ⦃x y : V⦄, G.Walk x y → Prop) : Finset ℕ := by
  classical
  exact (Finset.range (G.edgeFinset.card + 1)).filter fun n =>
    ∃ x y, ∃ p : G.Walk x y, P p ∧ p.edges.length = n

omit [DecidableEq V] in
private theorem zero_mem_trailLengthSetFor
    (P : ∀ ⦃x y : V⦄, G.Walk x y → Prop)
    (u : V)
    (hPnil : P (SimpleGraph.Walk.nil : G.Walk u u)) :
    0 ∈ trailLengthSetFor (G := G) (P := P) := by
  classical
  unfold trailLengthSetFor
  simp
  exact ⟨u, u, SimpleGraph.Walk.nil, hPnil, by simp⟩

private noncomputable def maxTrailLengthFor
    (P : ∀ ⦃x y : V⦄, G.Walk x y → Prop)
    (u : V)
    (hPnil : P (SimpleGraph.Walk.nil : G.Walk u u)) : ℕ :=
  (trailLengthSetFor (G := G) (P := P)).max'
    ⟨0, zero_mem_trailLengthSetFor (G := G) (P := P) u hPnil⟩

private theorem trail_length_le_maxTrailLengthFor
    (P : ∀ ⦃x y : V⦄, G.Walk x y → Prop)
    {u : V}
    (hPnil : P (SimpleGraph.Walk.nil : G.Walk u u))
    (hP_trail : ∀ ⦃x y : V⦄ (p : G.Walk x y), P p → p.IsTrail)
    {x y : V} {p : G.Walk x y}
    (hp : P p) :
    p.edges.length ≤ maxTrailLengthFor (G := G) (P := P) u hPnil := by
  classical
  unfold maxTrailLengthFor
  apply Finset.le_max'
  unfold trailLengthSetFor
  rw [Finset.mem_filter]
  constructor
  · rw [Finset.mem_range]
    exact Nat.lt_succ_of_le (hP_trail p hp).length_edges_le_card_edgeFinset
  · exact ⟨x, y, p, hp, rfl⟩

private theorem exists_maximalTrailFor
    (P : ∀ ⦃x y : V⦄, G.Walk x y → Prop)
    (u : V)
    (hPnil : P (SimpleGraph.Walk.nil : G.Walk u u))
    (hP_trail : ∀ ⦃x y : V⦄ (p : G.Walk x y), P p → p.IsTrail) :
    ∃ x y, ∃ p : G.Walk x y,
      P p ∧
      ∀ ⦃a b : V⦄ (q : G.Walk a b), P q →
        q.edges.length ≤ p.edges.length := by
  classical

  have hmax_mem :
      maxTrailLengthFor (G := G) (P := P) u hPnil ∈ trailLengthSetFor (G := G) (P := P) := by
    unfold maxTrailLengthFor
    exact Finset.max'_mem _ _

  unfold trailLengthSetFor at hmax_mem
  rw [Finset.mem_filter] at hmax_mem
  rcases hmax_mem.2 with ⟨x, y, p, hpP, hpLen⟩

  refine ⟨x, y, p, hpP, ?_⟩
  intro a b q hqP
  have hqLe :
      q.edges.length ≤ maxTrailLengthFor (G := G) (P := P) u hPnil :=
    trail_length_le_maxTrailLengthFor
      (G := G) (P := P) hPnil hP_trail hqP
  rw [hpLen]
  exact hqLe
...
private theorem exists_closed_trail_disjoint_of_unused_incident_edge
  ...
  obtain ⟨a, z, q, hqP, hmax⟩ :=
    exists_maximalTrailFor
      (G := G)
      (P := fun ⦃a b : V⦄ (q : G.Walk a b) =>
  ...
  have hqmax : MaximalTrailAvoidingFrom p q := by
    constructor
    · exact hqP.2.1
    constructor
    · exact hqP.2.2
    · intro z r hrTrail hrDisj
      exact hmax r ⟨rfl, hrTrail, hrDisj⟩
...
private theorem exists_isEulerian_of_connected_card_oddDegree_eq_two
    (hconn : G.Connected)
    (hodd : Fintype.card {v : V | Odd (G.degree v)} = 2) :
  ...
  obtain ⟨u, v, p, hpTrail, hmax⟩ :=
    exists_maximalTrailFor
      (G := G)
      (P := fun ⦃x y : V⦄ (p : G.Walk x y) => p.IsTrail)
      u₀
      (by simp)
      (by
        intro x y p hp
  ...
  have hpmax : MaximalTrail p := by
    constructor
    · exact hpTrail
    · intro a b q hqTrail
      exact hmax q hqTrail
...
theorem exists_isEulerian_of_connected_card_oddDegree_eq_zero_or_two
  ...
  · obtain ⟨u₀⟩ := hconn.nonempty
    ...
    exact ⟨u, u, p, hpmax.is_eulerian_of_connected hconn⟩
